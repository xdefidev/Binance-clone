          ...
          <div className="w-full h-full m-auto flex gap-5 flex-col sm:flex-row">
            <div className="flex-1 max-h-full h-[450px]">
              <div className="sm:hidden w-full h-[250px] overflow-hidden before:absolute before:bottom-0 before:w-[66%] before:h-[2px] before:dark:bg-[#272A2E] before:bg-[#eaecef] before:left-2/4 before:-translate-x-2/4 relative">
                <div
                  className="bg-no-repeat bg-center bg-top bg-[length:auto_170%] w-full h-full "
                  style={{ backgroundImage: `url(${mobileTrade})` }}
                ></div>
              </div>

              <div className="hidden md:hidden sm:block w-full h-full relative">
                <div
                  className="absolute bg-no-repeat bg-[length:auto_100%] bg-center w-full h-full "
                  style={{ backgroundImage: `url(${mobileTrade})` }}
                ></div>
                <div
                  className="bg-no-repeat bg-center bg-[length:auto_50%] w-full h-full absolute bottom-50 "
                  style={{ backgroundImage: `url(${mobileFlat})` }}
                ></div>
              </div>

              <div className="hidden md:block w-full h-full relative">
                <div
                  className="bg-no-repeat bg-left bg-[length:auto_85%] h-full w-full z-20 relative"
                  style={{ backgroundImage: `url(${desktopTrade})` }}
                >
                  <div className="w-full h-full">
                    <div className="relative w-full h-full">
                      <div
                        className="absolute right-0 bg-no-repeat bg-[length:200px]  bg-center max-w-full w-[200px] h-full "
                        style={{ backgroundImage: `url(${mobileTrade})` }}
                      ></div>

                      <div
                        className="absolute -right-1 bottom-44 translate-y-2/4 bg-no-repeat bg-center bg-[length:210px] max-w-full w-[210px] h-[180px] "
                        style={{ backgroundImage: `url(${mobileFlat})` }}
                      ></div>
                    </div>
                  </div>
              ...
              <div className="flex flex-col-reverse sm:flex-col gap-y-7 space-y-5">
                <div className="flex flex-col sm:flex-row items-center sm:space-x-5 space-y-4">
                  ...
                  <div className="space-y-2 w-full text-center sm:text-left">
                    <p className="font-unbounded dark:text-gray-200 sm:dark:text-gray-100 text-md sm:text-xl">
                      Scan to Download
                    </p>
                    <p className="font-unbounded hidden sm:block text-gray-600 dark:text-gray-300 text-xl font-semibold">
                      iOS & Android
                    </p>
                  </div>
                </div>
                <div className="grid grid-cols-3 sm:grid-cols-2 md:grid-cols-3 gap-4 text-sm text-center text-gray-700 dark:text-gray-300">
                  <div className="group cursor-pointer">
                    <div className="group-hover:bg-[#f5f5f5] dark:group-hover:bg-[#2b3139] p-4 rounded-md space-y-2">
                      <AiFillApple
                        size={35}
                        className="m-auto dark:fill-white fill-gray-800"
                      />
                      <p>
                  ...
                  <div className="group cursor-pointer">
                    <div className="group-hover:bg-[#f5f5f5] dark:group-hover:bg-[#2b3139] p-4 rounded-md space-y-2">
                      ...
                      />
                      <p>
                        Android Apk<small>(coming soon)</small>
                      </p>
                    </div>
                  </div>
