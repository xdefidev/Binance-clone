...
const Navbar = () => {
  ...
  return (
    <div className="bg-white dark:bg-[#000] dark:text-white text-gray-800 transition-colors duration-100">
          ...
          <div className=" hidden lg:flex justify-between w-full text-sm lg:text-[14px]">
            <ul className="flex space-x-3 items-center w-full ">
              {NavMainData.map((data, index) =>
                data?.item === "Products" ? (
                  <li
                    className="ml-[15px] mr-[15px] relative group h-full py-5 group"
                    key={index}
                  >
                    <a
                      href="#"
                    ...
                    {data?.subItem && (
                      <ul className="shadow-md absolute top-[69px] p-7 rounded-md opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all bg-white flex flex-wrap gap-5 w-[800px] dark:bg-[#1e2329]">
                        {data?.subItem.map((data, index) => (
                          <li
                            key={index}
                            className="list-none flex-auto w-[30%] "
                          >
                            <a
                              href="#"
                              className="block w-full font-unbounded "
                            >
                              {data?.item}
                              <span className="block text-xs text-gray-500 font-unbounded ">
                                {data?.desc}
                              </span>
                            </a>
                          </li>
                  ...
                  <li
                    className="flex items-center relative group m-0 h-full py-5 group"
                    key={index}
                  >
                    <a
                      href="#"
                    ...
                    >
                      ...
                      {data?.subItem && (
                        <MdOutlineArrowDropDown
                          size={24}
                          className="fill-[#848e9c] group-hover:rotate-180 transition-all"
                        />
                      )}
                    </a>
                    {data?.subItem && (
